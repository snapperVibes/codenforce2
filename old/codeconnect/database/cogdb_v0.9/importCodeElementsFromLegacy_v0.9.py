...
def logerrorviolation(malformedrow):
    with open(VIOLATION_LOG_FILE, "ab") as outfile:
        writer = csv_utils.UnicodeWriter(
            outfile, delimiter="|", encoding=CSV_FILE_ENCODING
        )
...
def get_db_conn():
    ...
    db_conn = psycopg2.connect(
        database="cogdb", user="sylvia", password="c0d3", host="localhost", port=20000
    )
    return db_conn
